...
--Bumer_32 modification for mineos
--------------------------------------------------------------------
local handlers = {}
...
local function listen(name, callback)
  ...
  local handle = event.addHandler(function(e1, e2, e3)
    if e1 == name then
      pcall(callback, e1, e2, e3)
    end
  end)
...
local function ignore(name, callback)
  local id = nil
  ...
  event.removeHandler(handlers[id].handle)
  table.remove(handlers, id)
end
...
local server_api = {
  ...
  reconnect = function(self)
    ...
    listen('internet_ready', self.__listener)
  ...
  disconnect = function(self)
    ...
    self.__channels = {}
    self.__stream = ""
    ignore('internet_ready', self.__listener)
  end,
...
function stem.connect(address, port)
  ...
  server.__listener = function(_, _, id)
    server:__incoming(id)
  end
